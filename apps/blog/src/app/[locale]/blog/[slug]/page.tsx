...
export default async function Post({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  return (
    <>
      ...
      {/* Heading numbering CSS counters (matching old site) */}
      ...
      <main>
              ...
              <div className="mt-6">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <a href={`mailto:?subject=${encodedTitle}&body=${encodedUrl}`}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs text-gray-600 bg-white/20 hover:bg-white/40 rounded-md transition-colors">
        ...
        {/* Mobile TOC — collapsible (visible on mobile) */}
        {headings.length > 0 && (
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 lg:hidden">
            <details className="bg-[#F4F6F9] rounded-xl border border-gray-200">
              <summary className="px-5 py-3 cursor-pointer text-sm font-semibold text-[#1B365D]">
                {tb('toc', 'Contents')}
              </summary>
              <div className="px-5 pb-4">
                <nav>
                  <ul className="space-y-1.5">
                    {headings.map((h, i) => (
                      <li key={i}>
                        <a href={`#${h.id}`} className={`block text-sm transition-colors hover:text-[#B8960C] ${h.level === 3 ? 'pl-4 text-gray-500' : 'text-gray-700 font-medium'}`}>{h.text}</a>
                      </li>
                    ))}
                  </ul>
                </nav>
                ...
                <div className="mt-12 pt-8 border-t border-gray-200">
                  ...
                  <ul className="space-y-2">
                    {references.map((ref, i) => (
                      <li key={i}>
                        <a href={ref.url} target="_blank" rel="noopener noreferrer" className="text-sm text-[#2563EB] hover:underline">{ref.title}</a>
                      </li>
                    ))}
                  </ul>
                </div>
                        ...
                        >
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-1 text-xs text-[#5F6F7F]">
                              {rp.category && (
                                <span className="px-2 py-0.5 bg-primary-navy/10 rounded-full text-[#1B365D]">{rp.category}</span>
                              )}
        ...
        <section className="py-16 bg-[#F4F6F9]">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <form className="w-full max-w-lg mx-auto bg-white rounded-xl border border-gray-200 p-6 sm:p-8 shadow-sm" action="https://api.web3forms.com/submit" method="POST">
              ...
              <button type="submit" className="w-full bg-gold hover:bg-gold/90 text-primary-navy font-bold py-3.5 px-4 rounded-lg transition-all shadow-md">
        ...
        <section className="py-16 bg-primary-navy">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="inline-block bg-gold hover:bg-gold/90 text-primary-navy font-semibold px-8 py-3 rounded-md transition-all hover:shadow-lg">
              {tb('ctaButton', 'Get Free Assessment')}
            </a>
          </div>
        </section>
      ...
      {/* Floating WhatsApp + Email + Back to top */}
      <FloatingButtons />
    </>
  );
}
