...
export default function WatchService({ result }: { result: any }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      <SectionTitle icon="🔭" label={t("sectionTrademarkWatchService")} />
      <p className="text-sm text-gray-700 mb-2">{ws.description}</p>
      <p className="text-xs font-semibold text-gray-500 uppercase mb-1">{t("watchIncludes")}</p>
      ...
      <div className="flex gap-4 text-xs text-gray-500"><span><strong>{t("labelFrequency")}:</strong> {ws.frequency}</span><span><strong>{t("labelCost")}:</strong> {ws.cost}</span></div>
    </div>
  )
}
